...
pub struct HealthResponse {
    ...
    #[serde(default)]
    pub active_jobs: i64,
    /// Utterance cache backend in use: `"sqlite"` (default) or `"redis"`.
    #[serde(default = "default_cache_backend")]
    pub cache_backend: String,
    /// Whether Redis cache support is configured (a `redis_url` is set
    /// in `server.yaml`).
    #[serde(default)]
    pub redis_cache_enabled: bool,
    /// Whether the Redis connection is currently alive and responding to pings.
    #[serde(default)]
    pub redis_cache_connected: bool,
