...
/// Command-owned spec for `utseg`.
pub(crate) const UTSEG_SPEC: CommandModuleSpec = CommandModuleSpec {
...
/// Build the command-owned utseg plan from a persisted runner snapshot.
pub(crate) fn build_plan(
    job: &RunnerJobSnapshot,
    config: &ServerConfig,
) -> BatchedInferDispatchPlan {
    debug_assert_eq!(job.dispatch.command, ReleasedCommand::Utseg);
    BatchedInferDispatchPlan::from_job(job, config)
}

/// Run the utseg command through the shared batched-text kernel.
