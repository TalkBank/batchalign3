...
/// Command-owned spec for `translate`.
pub(crate) const TRANSLATE_SPEC: CommandModuleSpec = CommandModuleSpec {
...
/// Build the command-owned translate plan from a persisted runner snapshot.
pub(crate) fn build_plan(
    job: &RunnerJobSnapshot,
    config: &ServerConfig,
) -> BatchedInferDispatchPlan {
    debug_assert_eq!(job.dispatch.command, ReleasedCommand::Translate);
    BatchedInferDispatchPlan::from_job(job, config)
}

/// Run the translate command through the shared batched-text kernel.
