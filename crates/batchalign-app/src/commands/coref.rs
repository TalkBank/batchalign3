...
/// Command-owned spec for `coref`.
pub(crate) const COREF_SPEC: CommandModuleSpec = CommandModuleSpec {
...
/// Build the command-owned coref plan from a persisted runner snapshot.
pub(crate) fn build_plan(
    job: &RunnerJobSnapshot,
    config: &ServerConfig,
) -> BatchedInferDispatchPlan {
    debug_assert_eq!(job.dispatch.command, ReleasedCommand::Coref);
    BatchedInferDispatchPlan::from_job(job, config)
}

/// Run the coref command through the shared batched-text kernel.
