...
/// Command-owned spec for `morphotag`.
pub(crate) const MORPHOTAG_SPEC: CommandModuleSpec = CommandModuleSpec {
...
/// Build the command-owned morphotag plan from a persisted runner snapshot.
pub(crate) fn build_plan(
    job: &RunnerJobSnapshot,
    config: &ServerConfig,
) -> BatchedInferDispatchPlan {
    debug_assert_eq!(job.dispatch.command, ReleasedCommand::Morphotag);
    BatchedInferDispatchPlan::from_job(job, config)
}

/// Run the morphotag command through the shared batched-text kernel.
