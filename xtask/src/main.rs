...
const SPECIAL_PATH_PREFIXES: &[(&str, &str)] = &[("pyo3/", "pyo3")];
const SPECIAL_PACKAGE_NAMES: &[(&str, &[&str])] = &[("batchalign-types", &["pyo3"])];
...
fn inspect_changes() -> Result<AffectedScope> {
    ...
    for path in changed {
        ...
        if let Some(package) = package_for_path(&path, &package_dirs) {
            ...
            for (special_package, targets) in SPECIAL_PACKAGE_NAMES {
                if package == *special_package {
                    for target in *targets {
                        special_targets.insert((*target).to_owned());
                    }
                }
            }
        }
...
fn run_special(mode: AffectedMode, target: &str) -> Result<()> {
    ...
    let manifest = repo_root().join("pyo3").join("Cargo.toml");
    let command: Vec<String> = match mode {
        AffectedMode::Packages => unreachable!(),
        AffectedMode::Check => vec![
            "cargo".to_owned(),
            "check".to_owned(),
